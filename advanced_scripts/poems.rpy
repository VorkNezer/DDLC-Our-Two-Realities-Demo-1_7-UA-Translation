...
#Define all of the poems
    ...
    poem_m5 = Poem(
    author = "monika",
    title = "Стрибучі почуття",
    text = """\
Я пливу в цій нескінченній арені,
...
Чотири стіни оточують, лише отвір щоб заглянути.
Потім він наповнюється.
    Повно,
    Спокійно,
    Заспокійливо,

Залишаючи з відкриттям і закриттям.
Чекаю на повернення,
    Чекаю, щоб стрибнути крізь.
"""
    ...
    title = "Сон",
    text = """\
Мені здається, я бачила тінь —
Тінь минулого життя.
Я не можу її ігнорувати, лише йду слідом,
Наче це моя доля.

Я бачу, як пролітають картинки.
Я можу лише дивитись,
Як жахливі друзі мої
Дозволяють речам зникати в порожнечі.

Мені здається, це була брехня,
Лише поганий і страшний сон.
Я не можу повірити, що вони можуть померти,
Краще нехай будуть вільні.
"""
    )
...
"""
    ...
    title = "Світанок",
    text = """\
Темрява падає ще один день,
Благаючи не пускати світло.
Вона ніколи не слухає,
Ніколи не вчиться.
Чому її треба відганяти.

Світло пробивається всередину.
Нетерпляче й турботливе,
Тепле й ніжне.
Тужить за думками й турботою.

Зі спалахом, який воно створює,
Воно більше не знає, що робити.
Темрява знову падає.
"""
    )
...
"""
    ...
    poem_fes = Poem(
        author = "mc",
        title = "Організація фестивалю",
        text = """\
Ранок (8:00-12:00) буде для заходів, які проводяться клубами.
Друга половина дня (13:00-17:00) буде для заходів, що проводяться класними керівниками.
Вечір (18:00-22:00) буде для концертів та інших заходів.

Обов'язково робіть перерви під час змін на заходах і насолоджуйтеся фестивалем!

Деякі вчителі також облаштували кіоски з їжею на вулиці, якщо хтось хоче піти туди замість заходу.

Розклад клубів: (інтервали 30 хвилин, 5 хвилин між ними)
    1) Літературний клуб
    2) Дебатний клуб
    3) Клуб мажореток
    4) Клуб аніме
    5) Спортивні клуби (по 1 короткій грі кожен)
    6) Шаховий клуб
    7) Клуб на відкритому повітрі
    8) Кулінарний клуб (працює і під час обіду)

Список вечірніх заходів:
    1) Шкільний оркестр (1 година)
    2) Клуб легкої музики (15 хвилин)
    3) Клуб айдолів (15 хвилин)
    4) Драматичний клуб (1.5 години)
    5) Вогнище (1 година)
...
screen poem(currentpoem, paper="paper"):
    ...
    viewport id "vp":
        ...
        vbox:
            ...
            elif currentpoem.author == "player":
                text "[currentpoem.title]\n\n[currentpoem.text]" style "player_text"
...
style yuri_text:
    font "gui/font/Elzevir.ttf"
    size 32
    color "#000"
    outlines []

style yuri_text_2:
    font "gui/font/Calligraph_Regular.ttf"
    size 40
    color "#000"
    outlines []

style yuri_text_3:
    font "gui/font/LC_Chalk.ttf"
    size 18
    color "#000"
    outlines []
    kerning -8
